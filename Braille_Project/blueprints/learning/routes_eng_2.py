...
# Configure logging (Move this to the main app if possible)
...
# Initialize the singleton HardwareBrailleKeyboard instance
keyboard = HardwareBrailleKeyboard.get_instance(port='/dev/ttyACM0')  # 실제 포트로 변경

@learning_bp.route('/led_control', methods=['POST'])
def led_control():
    """
    ...
    if keyboard.buffered_mode and keyboard.input_buffer:
        ...
        if input_buffer:
            ...
            if entered_word == target_word:
                ...
                # Optionally, you can schedule LED feedback similar to the Korean version
                ...
                # LED 점등을 위한 피드백 생성
                # 예시: 각 점자 그룹에 대해 LED 켜기 및 끄기 명령을 큐에 추가
                # braille_feedback는 "1,2,4 for a, 3,4 for n" 형태이므로, 이를 파싱
                braille_groups = braille_feedback.split(", ")
                for index, group in enumerate(braille_groups):
                    leds_part = group.split(" for ")[0]  # "1,2,4"
                    led_numbers = [int(num) for num in leds_part.split(",")]
                    
                    # LED 켜기 명령
                    Timer(index * 2.0, lambda leds=led_numbers: keyboard.queue_led_command(leds, action='ON')).start()
                    logging.debug(f"Scheduled ON command for LED group {led_numbers}")

                    # LED 끄기 명령 (2초 후)
                    Timer((index * 2.0) + 2.0, lambda leds=led_numbers: keyboard.queue_led_command(leds, action='OFF')).start()
                    logging.debug(f"Scheduled OFF command for LED group {led_numbers}")
